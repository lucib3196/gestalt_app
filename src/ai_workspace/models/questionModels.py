...
class Derivation(BaseModel):
    derivation_name: str = Field(
        ...,
        title="Title of the derivation",
        description="The name of the derivation should be consise and cleary define what the derivation aims to prove",
    )
    derivation_steps: List[Section] = Field(
        default_factory=list, title="Titles and descriptions for each derivation step"
    )
    source: Union[str, int, None] = Field(
        ...,
        description="The source of where the derivation was gathered ideally it is a number of the page it was extracted from if it is present",
        title="Source of where the derivation was extracted from the lecture content",
    )
    requires_image: bool = Field(
        ...,
        description="Wether the derivation requires an image to fully understand",
        title="A boolean flag to determine if an image is required for the derivation",
    )
    completeness: bool = Field(
        ...,
        description="Wether the derivation is complete or incomplete",
        title="Wether the derivation is complete or incomplete",
    )

    @property
    def as_str(self) -> str:
        derivation_steps = "\n\n".join(
            derivation_step.as_str for derivation_step in self.derivation_steps
        )
        source = f"\nSource: {self.source}" if self.source else ""
        return f"# {self.derivation_name}\n {source} \n\n{derivation_steps}".strip()
