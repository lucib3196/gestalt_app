...
export const PackageContents: React.FC = () => {
  ...
  let fileRows: TableCell[][] | undefined;
  let questionHeaders: TableHeader[] | undefined;
  let questionRows: TableCell[][] | undefined;

  if (files && files.length > 0) {
    const result = createTableData<File>(fileColumns, files, (key, q) => {
      if (key === "filename")
  ...
  if (questions && questions.length > 0) {
    const result = createTableData<Question>(
      questionColumns,
      questions,
      (key, q) => {
        if (key === "title") router.push(`/packages/folder/${q.id}`);
  ...
  return (
    <section className="p-6 bg-white rounded-lg shadow">
      ...
      {files && files.length > 0 && fileHeaders && fileRows && (
        <Table caption="Question Files" header={fileHeaders} data={fileRows} />
      )}
      {questions && questions.length > 0 && questionHeaders && questionRows && (
        <Table
